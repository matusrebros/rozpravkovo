<footer>
	<a href="/">
			<img class="top-logo" src="/assets/img/main-logo.svg">
	</a>
		<nav>
		<ul>
				<li>
					<a href="/ponuka-nehnutelnosti">Ponuka Nehnuteľností</a>
				</li>
				<li>
					<a class="speceial-one" href="/prenajom-naveky">Prenájom Naveky</a>
					<span>Time Sharing</span>
				</li>
				<li>
					<a href="/aktivity">Aktivity</a>
				</li>
				<li>
					<a href="/o-projekte">O projekte</a>
				</li>
				<li>
					<a href="/novinky">Novinky</a>
				</li>
				<li>
					<a href="/galeria">Galéria</a>
				</li>
				<li>
					<a href="/kontakt">Kontakt</a>
				</li>
			</ul>
		</nav>
		<div class="contacts">
			<a href="#" class="contact">
				<img src="/assets/img/footer-phone.svg">
				<span>+ 421 911 123 456</span>
			</a>
			<a href="#" class="contact">
				<img src="/assets/img/footer-mail.svg">
				<span>info@rozpravkovotatry.sk</span>
			</a>
			<a target="_blank" href="https://goo.gl/maps/Vga8ajWF4HxzUyYx7" class="contact">
				<img src="/assets/img/footer-address.svg">
				<span>Adresa 10, Tatranská Lomnica</span>
			</a>
			<a target="_blank" href="https://www.facebook.com/chatyrozpravkovo/" class="contact">
				<img src="/assets/img/footer-fb.svg">
				<span>Facebook</span>
			</a>
		</div>
	</footer>

	<div class="copyright">
		<div class="container">
			<span>Designed by</span>
			<a target="_blank" href="https://www.behance.net/samuelmucha">
				<img src="/assets/img/brandly-logo.svg">
			</a>
			<span>built by</span>
			<a href="mailto:info@dpmarketing.sk">
				<img src="/assets/img/dp-logo.svg">
			</a>
		</div>
	</div>

	<script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
	<script src="/assets/js/simple-lightbox.jquery.min.js"></script>
	<script src="/assets/js/script.js"></script>

</body>
</html>
